...
class LoanPaymentController extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($state)) {
            ...
            $customersPend = LoanPayment::join('customers', 'customers.id', '!=', 'loan_payments.customer_id')->where([['user_id', auth()->user()->id], ['date', [$state->date, date('Y-m-d')]]])->get();
        } else {
            $customerspays = [];
            $loanPay = [];
            $pendingpayment = [];
    ...
    public function create()
    {
        ...
        $customers = AssignPayment::join('customers', 'customers.id', '=', 'assign_payments.customer_id')->where('user_id', 1)->get();
